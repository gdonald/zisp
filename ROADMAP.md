...
- [x] 2.3. Special forms
  ...
  - [x] 2.3.9. `block` and `return-from`
  - [x] 2.3.10. `tagbody` and `go` including: backward jumps (loop construction), forward jumps, jumps out of nested `tagbody` (target tag in outer scope), `go` from inside an `unwind-protect` cleanup (cleanup runs, jump completes), `go` to a tag in dynamically-distant scope (must error if tag is no longer in scope). Acceptance: 8 cases in `tests/lisp/tagbody-corpus.lisp` diffed against SBCL
  - [x] 2.3.11. `catch` and `throw` including: throw past 5+ frames, throw to a tag established by an outer caller, throw from inside an `unwind-protect` (cleanup runs first, throw completes), throw to non-existent tag (signals `control-error`), interaction with multiple values (thrown values match `multiple-value-list` semantics). Acceptance: 6 cases in `tests/lisp/catch-throw-corpus.lisp`
  - [x] 2.3.12. `unwind-protect` (full version waits for Phase 6, stub now)
  - [x] 2.3.13. `the` (type declaration — accept and ignore for now)
  - [x] 2.3.14. `declare` parsing (accept and ignore most)
  - [x] 2.3.15. `multiple-value-bind`, `multiple-value-call`, `values`, `values-list`, `multiple-value-prog1`, `multiple-value-list`. `multiple-value-call` MUST handle multiple producers correctly: `(multiple-value-call #'list (values 1 2) (values 3 4))` returns `(1 2 3 4)`. Acceptance: 8 cases in `tests/lisp/multiple-values-corpus.lisp` covering 0-value producers, 1-value producers, many-value producers, and mixing producers with regular forms in the same call
  - [x] 2.3.16. `eval-when` (Phase 3 makes this load-bearing)
- [x] 2.4. Function calls and lambda lists
  - [x] 2.4.1. Required parameters
  - [x] 2.4.2. `&optional` with default forms and supplied-p flags
  - [x] 2.4.3. `&rest`
  - [x] 2.4.4. `&key` with default forms and supplied-p. Includes: `&allow-other-keys` in lambda list, `:allow-other-keys t` from caller side overriding lambda-list strictness, duplicate keys (first wins per CLHS 3.4.1.4.1), odd-argument-count errors at call site, interaction with `&rest` (rest list contains the keyword/value pairs). Acceptance: 25 cases in `tests/lisp/key-args-corpus.lisp` covering each behavior independently; cross-tested against SBCL
  - [x] 2.4.5. `&aux`
  - [x] 2.4.6. Argument-count checking with proper error messages
  - [x] 2.4.7. Closures capturing lexical environment
  - [x] 2.4.8. Tail calls — `(defun f () (f))` runs 10 seconds without stack overflow (trampoline acceptable here; real TCO is 9.2.6, don't conflate)
  - [x] 2.4.9. Multiple return values plumbed through call sites
- [x] 2.5. Minimal built-in functions
  - [x] 2.5.1. `cons`, `car`, `cdr`, `caar` … `cddddr`, `first` … `tenth`, `nth`, `nthcdr`
  - [x] 2.5.2. `list`, `list*`, `append`, `reverse`, `nreverse`, `length`
  - [x] 2.5.3. `eq`, `eql`, `equal`, `equalp`
  - [x] 2.5.4. `atom`, `consp`, `listp`, `null`, `endp`, `symbolp`, `numberp`, `integerp`, `stringp`
  - [x] 2.5.5. Fixnum arithmetic: `+`, `-`, `*`, `/`, `mod`, `rem`, `1+`, `1-`, `abs`, `min`, `max`
  - [x] 2.5.6. Comparisons: `=`, `/=`, `<`, `>`, `<=`, `>=`, `zerop`, `plusp`, `minusp`, `oddp`, `evenp`
  - [x] 2.5.7. `not`, `and` (special form), `or` (special form), `when`, `unless`, `cond` (as macros once Phase 3 lands)
  - [x] 2.5.8. `funcall`, `apply`
  - [x] 2.5.9. `mapcar`, `mapc`, `mapcan` over lists, INCLUDING multi-list forms: `(mapcar #'+ '(1 2 3) '(10 20 30)) => (11 22 33)`; stops at shortest list. Sequence-generic `map`/`map-into` come at 4.2 once vectors exist
- [x] 2.6. REPL
  - [x] 2.6.1. Read–eval–print loop in `src/repl/`
  - [x] 2.6.2. `*`, `**`, `***` and `+`, `++`, `+++` history variables
  - [x] 2.6.3. Error → break loop with `:abort` and `:continue` options (full version Phase 6)
- [x] 2.7. Driver / CLI surface (Phase 2 expansion)
  - [x] 2.7.1. `--eval EXPR` / `-e` — read, eval, print, repeat for each occurrence
  - [x] 2.7.2. `--load FILE` / `-l` — load a Lisp file
  - [x] 2.7.3. `--batch` — no REPL after processing args; exit when done
  - [x] 2.7.4. `--quiet` / `-q` — suppress banner
  - [x] 2.7.5. `--script FILE` — treat first arg as script, remaining args bound to a variable
  - [x] 2.7.6. `--` separator: end-of-options
  - [x] 2.7.7. Exit code propagates from uncaught errors / `(quit N)`
- [x] 2.8. Minimal "batch" builtins required by the test harness
  - [x] 2.8.1. `load` (file path, no `:if-does-not-exist` yet)
  - [x] 2.8.2. `quit` / `exit` with optional integer status
  - [x] 2.8.3. `*standard-output*` writes via Zig's I/O
  - [x] 2.8.4. `format t` with at least `~A`, `~S`, `~D`, `~%`
  - [x] 2.8.5. `*features*` populated (`:zisp`, `:ansi-cl`, host OS, host arch)
- [x] 2.9. Test harness goes live
  - [x] 2.9.1. Fill in `--eval` invocation TODO in `tests/run-ansi.sh`
  - [x] 2.9.2. Parse `PASS=N FAIL=M` from output, accumulate per category
  - [x] 2.9.3. Print per-phase summary (mapping categories to ROADMAP phases)
  - [x] 2.9.4. Non-zero exit on any failures
  - [x] 2.9.5. CI runs `zig build ansi-test` and tracks the pass rate
...
- [x] 3.1. `defmacro` machinery
  - [x] 3.1.1. `defmacro` special form / top-level definer
  - [x] 3.1.2. Macro lambda lists with full destructuring. Split per feature:
    - [x] 3.1.2a. Single-level destructuring: required params, `&optional`, `&rest`, `&key`. 6 cases in `tests/lisp/destructuring/basic.lisp`
    - [x] 3.1.2b. `&body` with declare/docstring extraction at body position. 4 cases
    - [x] 3.1.2c. `&whole` captures the full unparsed form. 3 cases
    - [x] 3.1.2d. `&environment` captures the macroexpansion env (verified by nested `macroexpand` inside the macro body). 3 cases
    - [x] 3.1.2e. Nested destructuring patterns (one level deep). 5 cases
    - [x] 3.1.2f. Nested patterns to arbitrary depth + recursive `&optional`/`&rest`/`&key` inside nested patterns. 5 cases
    - [x] 3.1.2g. Integration: `vendor/ansi-test/data-and-control-flow/destructuring-bind*.lsp` ≥ 95% pass rate; 30-form `tests/lisp/macro-destructuring-corpus.lisp` expands `equal` to SBCL
  - [x] 3.1.3. Macro expansion happens before evaluation in `eval`
  - [x] 3.1.4. `macroexpand-1` and `macroexpand` built-ins
  - [x] 3.1.5. `*macroexpand-hook*`
  - [x] 3.1.6. Macroexpansion preserves source positions. After `macroexpand-1`, the position table for the result satisfies: (a) forms passing through verbatim from the input retain their pre-expansion position; (b) forms synthesized by the macro carry the macro definition form's position. Acceptance: 10 NON-TRIVIAL macros in `tests/lisp/macroexpand-positions.lisp` (each synthesizes new forms, wraps user code in additional structure, or nests another macro expansion — pass-through identity macros disqualified); each is macroexpanded and the result's position table walked to assert the rule above. Verified by direct position-table API. Runtime-error integration (errors inside expansions report the user's call-site) is folded into 6.5.3
- [x] 3.2. Backquote
  - [x] 3.2.1. Backquote → `(quasiquote ...)` reader transform
  - [x] 3.2.2. Quasiquote expander producing equivalent `cons`/`list`/`append` calls
  - [x] 3.2.3. `,@` splicing — `(equal (eval ``(a ,@'(b c) d)) '(a b c d))` and 20 similar cases pass
  - [x] 3.2.4. Nested backquote. Split into 4 milestones:
    - [x] 3.2.4a. Implement basic backquote algorithm at depth 1 (`'`, `,`, `,@`). 10 simple forms in `tests/lisp/backquote-basic.lisp` expand correctly
    - [x] 3.2.4b. Implement nesting (depth 2+): each backquote increments a depth counter, each unquote decrements; only depth-0 unquotes evaluate. 10 nested forms expand correctly
    - [x] 3.2.4c. CLtL2 Appendix C examples — every form in sections C.1–C.4 (~12 forms, the Steele test cases — historically tricky) expands `equal` to SBCL's expansion
    - [x] 3.2.4d. Extended corpus — 38 additional forms with nesting depth ≥3 mixing `,`/`,@` at multiple levels, all `equal` to SBCL
  - [x] 3.2.5. Edge cases: `` `,x ``, `` `,@x `` at top level error correctly; `` ``(,(,x)) `` evaluates without crashing
- [x] 3.3. Hygiene helpers
  - [x] 3.3.1. `gensym` with `*gensym-counter*`
  - [x] 3.3.2. `gentemp`
  - [x] 3.3.3. Documented gotcha: CL macros are *not* hygienic; this is by design
- [x] 3.4. Compile-time control
  - [x] 3.4.1. `eval-when` per CLHS 3.2.3.1. Split:
    - [x] 3.4.1a. Top-level `eval-when` with each single situation: `:compile-toplevel`, `:load-toplevel`, `:execute` (3 cases)
    - [x] 3.4.1b. Top-level `eval-when` with situation combinations (`:compile-toplevel :load-toplevel`, etc.) — all 7 non-empty subsets (7 cases)
    - [x] 3.4.1c. `eval-when` nested inside `progn` at top level — same 7 subsets (7 cases)
    - [x] 3.4.1d. `eval-when` in non-top-level position behaves per CLHS rules (situations other than `:execute` ignored). 4 cases
    - [x] 3.4.1e. Deprecated situation keywords (`compile`/`load`/`eval`) accepted with warning. 3 cases
    - [x] 3.4.1f. Full state-table coverage: 21 cases in `tests/lisp/eval-when-corpus.lisp` (one per cell of 3-situation × 7-position table); each produces exactly SBCL's output. Diff is empty
  - [x] 3.4.2. Top-level form processing distinguishes file-compile from REPL (required for 3.4.1 to be testable). `*compile-file-pathname*`, `*compile-file-truename*`, `*load-pathname*`, `*load-truename*` populated correctly during their respective operations
- [x] 3.5. Standard macros implementable now in Lisp (each is one session — implement, test, commit)
  - [x] 3.5.1a. `when`, `unless`
  - [x] 3.5.1b. `cond`
  - [x] 3.5.1c. `case`, `ecase`, `ccase`
  - [x] 3.5.1d. `typecase`, `etypecase`, `ctypecase`
  - [x] 3.5.2. `and`, `or` reimplemented as macros over `if`
  - [x] 3.5.3. `prog1`, `prog2`
  - [x] 3.5.4a. `push`, `pop`
  - [x] 3.5.4b. `pushnew` (with `:test` and `:key`)
  - [x] 3.5.5. `incf`, `decf` (require `setf` infrastructure)
- [x] 3.6. `setf`
  - [x] 3.6.1. `setf` as a macro dispatching on the place form
  - [x] 3.6.2. Built-in setf expanders (one session each):
    - [x] 3.6.2a. `(setf car)`, `(setf cdr)`
    - [x] 3.6.2b. `(setf nth)`, `(setf elt)`
    - [x] 3.6.2c. `(setf aref)` (for vectors; multi-dim deferred to 4.3)
    - [x] 3.6.2d. `(setf gethash)`
    - [x] 3.6.2e. `(setf symbol-value)`, `(setf symbol-function)`, `(setf symbol-plist)`
    - [x] 3.6.2f. `(setf get)` (property list)
  - [x] 3.6.3. `defsetf` short form, then `defsetf` long form. Long form is NOT skippable: a `defsetf` for `(my-getter obj k)` with side-effecting subforms must call each subform exactly once
  - [x] 3.6.4. `define-setf-expander` (long form). Acceptance: `tests/lisp/setf-expander-corpus.lisp` contains 5 expanders covering: side-effect ordering (each subform evaluated once, left-to-right), multiple store-variables (e.g. for `gethash` returning two values), environment capture, getting and setting share computed temporary, and a generic-function-style place
  - [x] 3.6.5. `get-setf-expansion` returns the 5-value protocol (temps, vals, store-vars, store-form, access-form) per CLHS 5.1.1.2
- [ ] 3.7. ansi-test framework boots
  - [x] 3.7.1. `vendor/ansi-test/rt-package.lsp` loads without error
  - [x] 3.7.2. `vendor/ansi-test/rt.lsp` loads — defines `deftest`, `do-tests`, `pending-tests`
  - [ ] 3.7.3. `vendor/ansi-test/gclload1.lsp` runs end-to-end
  - [x] 3.7.4. `(deftest trivial 1 1)` registers, `(do-tests)` reports it as passing
  - [x] 3.7.5. The `:cl-test` package can be entered
...
- [ ] 4.1. Strings
  ...
  - [x] 4.1.4. `char`, `schar`, `(setf char)`, `(setf schar)`
  - [x] 4.1.5. `string-upcase`, `string-downcase`, `string-capitalize` and destructive variants
...
- [x] 4.2. Sequences (generic over lists/vectors/strings)
  ...
  - [x] 4.2.3. `reduce`, `count`, `count-if`, `count-if-not`
  - [x] 4.2.4. `find`, `find-if`, `find-if-not`, `position`, `position-if`
  - [x] 4.2.5. `remove`, `remove-if`, `delete`, `delete-if` (and `-not` variants)
  - [x] 4.2.6. `substitute`, `nsubstitute`
  - [x] 4.2.7. `sort`, `stable-sort`, `merge`. `sort` is destructive, not required to be stable; `stable-sort` IS stable. Acceptance: `tests/lisp/stable-sort.lisp` sorts 1000 `(key index)` pairs by key — for every pair of equal-key elements, the index ordering is preserved (stability gate). `:key` and `:test` honored on all three; `merge` preserves stability when both inputs are sorted
  - [x] 4.2.8. `concatenate`, `reverse`, `nreverse`
- [x] 4.3. Vectors and arrays
  - [x] 4.3.1. One-dimensional vectors with element-type and fill-pointer support
  - [x] 4.3.2. `make-array` with `:element-type`, `:initial-element`, `:initial-contents`, `:adjustable`, `:fill-pointer`, `:displaced-to`. Acceptance: `tests/lisp/array-options-corpus.lisp` covers the (displaced/adjustable/fill-pointer) × (general/specialized element-type) interaction matrix — at least 12 cases — including: displaced-to an adjustable array, adjusting a displaced array, fill-pointer on a displaced array, displaced-to with an offset, and the spec-required errors for invalid combinations
  - [x] 4.3.3. Multi-dimensional arrays (row-major layout)
  - [x] 4.3.4. `aref`, `(setf aref)`, `row-major-aref`
  - [x] 4.3.5. `array-rank`, `array-dimensions`, `array-total-size`
  - [x] 4.3.6. Specialized arrays (at least `bit-vector` and `(unsigned-byte 8)`)
  - [x] 4.3.7. `vector-push`, `vector-push-extend`, `vector-pop`
  - [x] 4.3.8. `adjust-array`
- [x] 4.4. Hash tables
  - [x] 4.4.1. `make-hash-table` with `:test`, `:size`, `:rehash-size`, `:rehash-threshold`
  - [x] 4.4.2. Tests: `eq`, `eql`, `equal`, `equalp`
  - [x] 4.4.3. `gethash`, `(setf gethash)`, `remhash`, `clrhash`
  - [x] 4.4.4. `maphash`, `with-hash-table-iterator`
  - [x] 4.4.5. `hash-table-count`, `hash-table-size`, `hash-table-test`
- [x] 4.5. Numeric tower
  - [x] 4.5.1. Bignums (arbitrary-precision integers). Split:
    - [x] 4.5.1a. Decision: wrap Zig's `std.math.big.int` vs hand-rolled limbs. Document in `docs/bignum-impl.md`
    - [x] 4.5.1b. `bignum` heap type defined; allocator wired through GC; `print-object` produces decimal output
    - [x] 4.5.1c. Bignum + bignum and bignum + fixnum addition, subtraction
    - [x] 4.5.1d. Bignum multiplication
    - [x] 4.5.1e. Bignum division: `floor`, `ceiling`, `truncate`, `round` (each returns quotient and remainder as multiple values)
    - [x] 4.5.1f. Bignum comparisons: `=`, `<`, `>`, `<=`, `>=` (mixed bignum/fixnum)
    - [x] 4.5.1g. Bit operations on bignums: `logand`, `logior`, `logxor`, `lognot`, `ash`, `integer-length`
  - [x] 4.5.2. Automatic promotion from fixnum on overflow in `+`, `-`, `*` (uses 4.5.1c–d). Verified: `(* most-positive-fixnum 2)` returns a bignum without crash
  - [x] 4.5.3. Integer division: `floor`, `ceiling`, `truncate`, `round` (with two return values)
  - [x] 4.5.4. Ratios as `(numerator . denominator)` reduced to lowest terms
  - [x] 4.5.5. Single-float and double-float (Zig's `f32`/`f64`) with shortest-round-trippable printing. Split into 4 milestones:
    - [x] 4.5.5a. Algorithm decision: `docs/float-printing.md` written choosing among Steele-White / Ryu / Grisu, with implementation effort, output quality, dependency cost compared. Sign-off required
    - [x] 4.5.5b. Basic float printing (any correct format, may produce too many digits): `(prin1 1.5)` produces `"1.5"`, `(prin1 1d0)` produces `"1.0d0"`. Pass: 50-value corpus formats without crash
    - [x] 4.5.5c. Shortest-round-trippable: `(read-from-string (prin1-to-string x)) = x` (bitwise) for the basic 50-value corpus
    - [x] 4.5.5d. Hard-case corpus: 1000 values including subnormals, denormals, smallest/largest representable, ±0.0, and values that historically expose Dragon4 bugs (`5e-324`, `2.2250738585072014e-308`, `1e23`). Round-trip bitwise for every value. NOT bail-able to `printf("%g", x)` — this corpus immediately catches it
  - [x] 4.5.6. Complex numbers including branch-cut compliance for `log`, `sqrt`, `asin`, `acos`, `atan`, `atanh`, `phase` per CLHS 12.1.5.3. Acceptance: `vendor/ansi-test/numbers/complex*.lsp` and `branch-cut*.lsp` ≥ 90% pass rate
  - [x] 4.5.7. Numeric contagion rules (integer + float → float)
  - [x] 4.5.8. `gcd`, `lcm`, `expt` (including edge cases: `(expt 0 0) => 1`, `(expt -2 1/2)` returns complex, `(expt 2 1000)` returns bignum), `isqrt`, `sqrt`, `log` (1-arg natural log; 2-arg arbitrary base; `(log 0)` signals `arithmetic-error`), `exp`, `sin`/`cos`/`tan`/`asin`/`acos`/`atan` (2-arg `atan` with proper quadrant handling), `sinh`/`cosh`/`tanh` and inverses. Acceptance: `vendor/ansi-test/numbers/` transcendental files (count, embed) ≥ 85% pass rate
  - [x] 4.5.9. `random` and `*random-state*`
- [x] 4.6. Characters
  - [x] 4.6.1. Character object distinct from fixnum
  - [x] 4.6.2. `char-code`, `code-char`, `char-name`, `name-char`
  - [x] 4.6.3. `alpha-char-p`, `alphanumericp`, `digit-char-p`, `upper-case-p`, `lower-case-p`
  - [x] 4.6.4. Full Unicode case mapping. Acceptance: 50 cases in `tests/lisp/unicode-cases.lisp` pass, MUST include: Turkish locale i/I/ı/İ pair behavior, German ß ↔ SS round-trip, Greek final sigma context (ς in word-final, σ elsewhere), ligatures (ﬃ → FFI), combining marks (composed vs decomposed equivalence), surrogate pair edge cases at the BMP boundary, and at least 5 cases from `SpecialCasing.txt` that Zig's stdlib doesn't handle (forces manual port of those rows). NOT bail-able to "ASCII + BMP" — the Turkish and ligature cases are explicit gates
- [x] 4.7. Pathnames and streams
  - [x] 4.7.1. `pathname` type with all six components (host/device/directory/name/type/version). Physical pathnames only at this phase — logical pathnames explicitly stubbed with a `feature-not-implemented` error and tracked as 4.7.9 below. NOT bail-able to "skip version" — the version component (`:newest`, `:wild`, integer) must round-trip through `make-pathname`/`pathname-version`
  - [x] 4.7.2. `make-pathname`, `merge-pathnames`, `namestring`, `parse-namestring`. `merge-pathnames` follows the "missing component" rules from CLHS 19.2.2.4 exactly. Acceptance: `vendor/ansi-test/pathnames/` pass rate ≥ 85% on the non-logical-pathname subset; specifically, all 18 `merge-pathnames*.lsp` test cases pass
  - [x] 4.7.3. Stream protocol: input, output, bidirectional, character vs binary
  - [x] 4.7.4. `open`, `close`, `with-open-file` with full option matrix. Split into 5 milestones:
    - [x] 4.7.4a. Basic `open`/`close` with `:direction :input` reading a file; `with-open-file` ensures close on normal exit AND on error. 4 cases
    - [x] 4.7.4b. `:direction` variants: `:input`, `:output`, `:io`, `:probe`. 4 cases (one per direction)
    - [x] 4.7.4c. `:if-exists` matrix: `:supersede`, `:append`, `:overwrite`, `:rename`, `:rename-and-delete`, `:error`, `:new-version`, `nil`. 8 cases
    - [x] 4.7.4d. `:if-does-not-exist` matrix: `:create`, `:error`, `nil`. 3 cases
    - [x] 4.7.4e. `:element-type` (`character`, `(unsigned-byte 8)`, `(signed-byte 16)`) and `:external-format` (`:utf-8`, `:latin-1`). 6 cases including byte-stream round-trip and UTF-8 round-trip
  - [x] 4.7.5. `read-char`, `peek-char`, `unread-char`, `write-char`
  - [x] 4.7.6. `read-line`, `write-line`, `write-string`
  - [x] 4.7.7. String streams: `with-input-from-string`, `with-output-to-string`
  - [x] 4.7.8. `*standard-input*`, `*standard-output*`, `*error-output*`, `*query-io*`, `*terminal-io*`
  - [x] 4.7.9. Logical pathnames. `logical-pathname-translations`, `translate-logical-pathname`, `load-logical-pathname-translations`. Acceptance: `vendor/ansi-test/pathnames/logical*.lsp` pass rate ≥ 80%. MUST land in Phase 4 — Phase 4 is incomplete until 4.7.9 is checked. NOT bail-able to "rarely used, defer to Phase 10"
- [x] 4.8. Packages
  - [x] 4.8.1. `package` type
  - [x] 4.8.2. Standard packages: `COMMON-LISP` (`CL`), `COMMON-LISP-USER` (`CL-USER`), `KEYWORD`
  - [x] 4.8.3. `defpackage`, `make-package`, `delete-package`
  - [x] 4.8.4. `in-package`, `*package*`
  - [x] 4.8.5. Symbol movement and conflict resolution per CLHS 11.1.1.2.5. Split per operation:
    - [x] 4.8.5a. `export` — internal symbol becomes external; symbol becomes accessible to packages that `use` this one
    - [x] 4.8.5b. `import` (no conflict case) — symbol becomes internal in the package; multiple imports of the same symbol are no-ops
    - [x] 4.8.5c. `import` conflict detection — importing a symbol when a different symbol of the same name is accessible signals `package-error`. 3 cases
    - [x] 4.8.5d. `unintern` — basic case removes symbol from package
    - [x] 4.8.5e. `unintern` of a shadowing symbol triggers conflict re-check among used packages. 2 cases
    - [x] 4.8.5f. `shadow` — creates new internal symbol if none exists, marks existing symbol as shadowing
    - [x] 4.8.5g. `shadowing-import` — resolves conflicts silently by replacing the inherited symbol
    - [x] 4.8.5h. Use-package conflict at `use-package` time — two packages exporting same name signals `package-error`. 3 cases
    - [x] 4.8.5i. Integration: 15-case `tests/lisp/package-conflict-corpus.lisp` covering all the above interactions; cross-tested against SBCL
  - [x] 4.8.6. `use-package`, `unuse-package`
  - [x] 4.8.7. Symbol resolution: internal vs external, `pkg:sym` vs `pkg::sym`
  - [x] 4.8.8. `find-symbol`, `intern`, `find-package`
  - [x] 4.8.9. `do-symbols`, `do-external-symbols`, `do-all-symbols`
- [ ] 4.9. Type system
  - [x] 4.9.1. `typep` and `subtypep`. Split:
    - [x] 4.9.1a. `typep` for atomic types: `null`, `cons`, `symbol`, `fixnum`, `bignum`, `integer`, `ratio`, `float`, `string`, `vector`, `array`, `hash-table`, `function`, `package`, `pathname`, `stream`, `character`, `t`, `nil`. One case per type
    - [x] 4.9.1b. `typep` for compound numeric types: `(integer 0 100)`, `(real * 1.0)`, `(unsigned-byte 8)`, `(signed-byte 16)`, `(mod N)`. 6 cases
    - [x] 4.9.1c. `typep` for compound combinators: `(or T T ...)`, `(and T T ...)`, `(not T)`, `(satisfies fn)`, `(member ...)`, `(eql x)`. 6 cases including nesting
    - [x] 4.9.1d. `typep` for `deftype`-defined types (composes recursively). 4 cases
    - [ ] 4.9.1e. `typep` integration: `vendor/ansi-test/types-and-classes/typep*.lsp` ≥ 95%
    - [x] 4.9.1f. `subtypep` two-value return per CLHS 4.3.2. Atomic-type lattice (e.g. `(subtypep 'integer 'number) => (t t)`). 10 cases
    - [x] 4.9.1g. `subtypep` for compound numeric types (e.g. `(subtypep '(integer 0 5) '(integer 0 10))`). 8 cases
    - [x] 4.9.1h. `subtypep` for compound combinators (`or`/`and`/`not` interactions). 10 cases
  ...
  - [x] 4.9.2. `deftype`
  - [x] 4.9.3. Type specifiers: atomic, compound (`(integer 0 100)`, `(or string null)`)
  - [x] 4.9.4. `check-type`, `coerce`
- [x] 4.10. Pretty printer
  - [x] 4.10.1. `pprint` writes to `*standard-output*` with `*print-pretty*` honored
  - [x] 4.10.2. `pprint-logical-block`, `pprint-newline`, `pprint-indent`, `pprint-fill`, `pprint-tab`
  - [x] 4.10.3. `*print-right-margin*` honored. Acceptance: 20-input corpus in `tests/lisp/pprint-corpus.lisp` MUST include: deeply-nested lists (depth ≥10), atoms wider than the right margin, mixed cons/vector/string in one form, recursive structures via `*print-circle*`, miser-mode triggers (forms too wide for right margin causing all-clauses-on-own-line). Output diffed against SBCL byte-for-byte
  - [x] 4.10.4. Pretty-print dispatch tables: `set-pprint-dispatch`, `copy-pprint-dispatch`
...
- [x] 5.1. Mark-and-sweep baseline
  - [x] 5.1.1. Mark bit added to `HeapObject` header (1 bit reserved in flags)
  - [x] 5.1.2. Free list. Split:
    - [x] 5.1.2a. Single global free list (simplest). Allocation pops from head; deallocation pushes to head
    - [x] 5.1.2b. Free-list integrity test: 10000 alloc/free cycles, assert no double-free, no corruption (`tests/lisp/freelist-stress.lisp`)
    - [x] 5.1.2c. Free list per size class (4 classes: 16/32/64/128 bytes). Larger requests go to a generic block allocator
    - [x] 5.1.2d. Per-size-class fragmentation test: 1000 allocs of each class, free half, allocs of same class succeed without growing heap
  - [x] 5.1.3. Mark phase. Split:
    - [x] 5.1.3a. Mark a single object: set its mark bit. Test: mark a cons, verify bit is set
    - [x] 5.1.3b. Recursive mark for cons cells (car + cdr). Test: mark root of 100-cell list, verify all cells marked
    - [x] 5.1.3c. Type-dispatch in mark: extend to symbol (value/function/plist), string (no children), vector (each element), hash-table (each k/v)
    - [x] 5.1.3d. Convert recursive mark to worklist (avoid Zig stack overflow on deep structures). Test: mark a 1M-cell list without crash
  - [x] 5.1.4. Sweep phase: reclaim unmarked AND coalesce adjacent free blocks. Acceptance: `tests/lisp/gc-fragmentation.lisp` allocates 10000 cells of varying sizes, frees alternate cells (creating worst-case fragmentation), runs GC. After GC, `(room)` reports free-block count ≤ `(/ allocated-count 100)` — i.e. ≥99% of adjacent free blocks merged. NOT bail-able to "size classes hide fragmentation"
  - [x] 5.1.5. Trigger heuristic: GC when allocated bytes since last GC > threshold
- [x] 5.2. Root scanning
  - [x] 5.2.1. Symbol table (every interned symbol's value/function cells)
  - [x] 5.2.2. Global environment frames
  - [x] 5.2.3. Active call stack — Lisp frames maintained explicitly (not Zig stack)
  - [x] 5.2.4. `*` `**` `***` and other REPL state
  - [x] 5.2.5. Open streams
  - [x] 5.2.6. Pinned objects (for FFI)
- [x] 5.3. Stack discipline
  - [x] 5.3.1. Lisp call stack as an explicit growable array of `Value`
  - [x] 5.3.2. Helper: "shadow stack" pattern for any Zig function holding live `Value`s during a possible GC
  - [x] 5.3.3. No Zig function holds a raw `*Cons` (or any heap pointer) across a possible-GC point. Enforced mechanically: every allocation function increments a generation counter; in `Debug` builds, every dereference of a held heap pointer checks the counter and aborts if it changed across an allocation. CI runs the full test suite under `Debug` so violations break the build. NOT bail-able to "I read the code carefully" — the runtime check is the gate
...
- [x] 5.5. Generational follow-up
  ...
  - [x] 5.5.3. Minor GC: copy survivors to tenured. Split into 4 milestones:
    ...
    - [x] 5.5.3a-iv. Update all roots after copy (symbol table, env frames, REPL state) to point at new locations. Test: a global variable bound to a nursery cons points to the tenured copy after GC
...
- [x] 5.6. Introspection and tuning
  ...
  - [x] 5.6.2. `(gc)` to force a collection
  - [x] 5.6.3. `*gc-verbose*`, `*gc-trigger*`
  - [x] 5.6.4. Stats: bytes allocated, GC time, pause histogram
...
- [ ] 8.1. Loop facility — done means: `vendor/ansi-test/iteration/loop*.lsp` pass rate ≥ 95% AND a 200-form corpus from "Practical Common Lisp" + CLHS section 6 expands identically to SBCL (`tests/lisp/loop-corpus.lisp`)
  - [x] 8.1.1. `loop` macro — simple form (body repeated forever)
  - [ ] 8.1.2. Iteration clauses — one session each:
    - [x] 8.1.2a. `for ... in` (list elements) and `for ... on` (cdr-walk)
    - [x] 8.1.2b. `for ... = ... then ...` (assignment with optional step form)
    - [x] 8.1.2c. `for ... from ... to ... by ...` (numeric, including `downto`, `below`, `above`)
    - [x] 8.1.2d. `for ... across` (vector elements)
    - [x] 8.1.2e. `for ... being the hash-keys/hash-values [using ...]`
    - [ ] 8.1.2f. `for ... being the symbols/external-symbols/present-symbols [of package]`
    - [x] 8.1.2g. Multiple parallel `for` clauses (terminate when any source exhausts)
  - [x] 8.1.3. Termination — one session each:
    - [x] 8.1.3a. `while`, `until`
    - [x] 8.1.3b. `repeat N`
    - [x] 8.1.3c. `always`, `never`, `thereis` (boolean accumulators that double as terminators)
  - [x] 8.1.4. Accumulation — one session each:
    - [x] 8.1.4a. `collect`, `collecting` (and the `appending` family: `append`, `nconc`)
    - [x] 8.1.4b. `count`, `counting`
    - [x] 8.1.4c. `sum`, `summing`
    - [x] 8.1.4d. `maximize`, `minimize` (require at least one value or return NIL — verify spec behavior)
  - [x] 8.1.5. `into` accumulators with explicit interaction tests: `collect ... into x` + `count ... into y` + `finally (return (list x y))`
  - [x] 8.1.6. `with` for local bindings (parallel and sequential)
  - [x] 8.1.7. `initially`, `finally` (multiple of each, ordering matters)
  - [ ] 8.1.8. `if`/`when`/`unless`/`else`/`end` clauses — nested conditionals with `and`/`it` references
  - [x] 8.1.9. `named` loops with `return-from`
  - [ ] 8.1.10. Type declarations: `for x fixnum ...` actually inform compilation (Phase 9)
- [ ] 8.2. Other iteration
  - [x] 8.2.1. `do`, `do*` with multiple stepped variables, end-test, result form, body. `do` evaluates step forms in PARALLEL (all step forms reference the old values); `do*` evaluates SEQUENTIALLY (each step form sees prior steps' new values). Acceptance: 8 cases in `tests/lisp/do-corpus.lisp` distinguishing parallel-vs-sequential semantics — including `(do ((a 1) (b a)) ...)` failing in `do` (unbound `a` in `b`'s init context per spec) but working in `do*`
