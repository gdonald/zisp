//! The collection that walks the whole heap.
//!
//! `tests/lisp/major-gc.lisp` holds the checks: each one signals an
...
const corpus_path = "tests/lisp/major-gc.lisp";
...
test "every check in the major collection corpus holds" {
...
test "the collector chose most of those collections itself" {
    const fx = try Fixture.init(testing.allocator);
    defer fx.deinit(testing.allocator);

    try zisp.builtins.system.loadPath(&fx.ev, corpus_path);
    // The corpus asks for three, and the rest are the collector's own.
    try testing.expect(fx.ev.major_count > 0);
    try testing.expect(fx.ev.gc_count > fx.ev.major_count);
}
