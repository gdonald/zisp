...
//!
//! Reads `tests/lisp/stable-sort.lisp` and evaluates each form. Every form
//! is self-checking and must come back T; the first one is the stability
//! gate over a thousand equal-key pairs.
...
const corpus_text = @embedFile("../lisp/stable-sort.lisp");
...
test "every form in the sorting corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("sort corpus form {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("sort corpus form {d}: expected T\n", .{checked});
            return error.TestUnexpectedResult;
        }
    }
    try testing.expectEqual(@as(u32, 6), checked);
}
