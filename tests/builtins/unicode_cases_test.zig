...
//!
//! Reads `tests/lisp/unicode-cases.lisp` and evaluates each form. Every
...
const corpus_text = @embedFile("../lisp/unicode-cases.lisp");
...
test "every case in the unicode corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("unicode case {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("unicode case {d}: expected T\n", .{checked});
            return error.TestUnexpectedResult;
        }
    }
    // The roadmap asks for fifty cases; the corpus carries more.
    try testing.expect(checked >= 50);
    try testing.expectEqual(@as(u32, 70), checked);
}
