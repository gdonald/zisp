//! The pretty printer.
//!
//! Reads `tests/lisp/pprint-corpus.lisp` and evaluates each form. Every
...
const corpus_text = @embedFile("../lisp/pprint-corpus.lisp");
...
test "every form in the pretty printer corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("pprint corpus form {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("pprint corpus form {d}: expected T\n", .{checked});
            return error.TestUnexpectedResult;
        }
    }
    // The roadmap asks for twenty inputs; the corpus carries more.
    try testing.expect(checked >= 20);
    try testing.expectEqual(@as(u32, 31), checked);
}
