...
//!
//! Reads `tests/lisp/array-options-corpus.lisp` and evaluates each form.
...
const corpus_text = @embedFile("../lisp/array-options-corpus.lisp");
...
test "every form in the make-array option corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("array corpus form {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("array corpus form {d}: expected T\n", .{checked});
