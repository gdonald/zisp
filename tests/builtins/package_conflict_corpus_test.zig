//! Package symbol movement and conflict resolution.
//!
//! Reads `tests/lisp/package-conflict-corpus.lisp` and evaluates each
...
const corpus_text = @embedFile("../lisp/package-conflict-corpus.lisp");
...
test "every form in the package conflict corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("package corpus form {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("package corpus form {d}: expected T\n", .{checked});
            return error.TestUnexpectedResult;
        }
    }
    // The roadmap asks for fifteen cases; the corpus carries more.
    try testing.expect(checked >= 15);
    try testing.expectEqual(@as(u32, 26), checked);
}
