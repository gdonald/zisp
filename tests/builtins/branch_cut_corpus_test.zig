//! Complex branch cuts.
//!
//! Reads `tests/lisp/branch-cut-corpus.lisp` and evaluates each form.
...
const corpus_text = @embedFile("../lisp/branch-cut-corpus.lisp");
...
test "every form in the branch-cut corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("branch-cut corpus form {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("branch-cut corpus form {d}: expected T\n", .{checked});
