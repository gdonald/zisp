//! merge-pathnames and the pathname components.
//!
//! Reads `tests/lisp/merge-pathnames-corpus.lisp` and evaluates each form.
//! Every form is self-checking and must come back T.
...
const corpus_text = @embedFile("../lisp/merge-pathnames-corpus.lisp");
...
test "every form in the merge-pathnames corpus evaluates to T" {
    ...
    while (try rd.read()) |form| {
        ...
        const got = fx.ev.eval(form) catch |e| {
            std.debug.print("merge corpus form {d}: eval error {s}\n", .{ checked, @errorName(e) });
            return e;
        };
        if (!got.equalsRaw(value.T)) {
            std.debug.print("merge corpus form {d}: expected T\n", .{checked});
            return error.TestUnexpectedResult;
        }
    }
    try testing.expectEqual(@as(u32, 25), checked);
}
